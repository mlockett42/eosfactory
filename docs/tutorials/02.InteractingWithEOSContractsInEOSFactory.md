...
## Change blockchain data dir if installed globally

**NOTE:** If you installed *EOSFactory* using `pip install eosfactory` into
your global package repository you need to change the location of the blockchain
data directory to one you have write access to.

```python
from eosfactory.core.config import *
set_config_json_path('/tmp/localnode/')
```

The directory must exist.
```python
import os
os.mkdir('/tmp/localnode/')
```

Your config.json file must point a location that your user has write access to.
Save the below to `/tmp/localnode/config.json`
```json
{
  "LOCAL_NODE_DATA_DIR":"/tmp/localnode/"
}
```
